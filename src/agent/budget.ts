...
export interface BudgetView { messages: ChatMessage[]; result: BudgetResult; }
...
export function contextBudgetView(
  ...
  let historyTokens = estimateMessages(view, charsPerToken);
  let prunedChars = 0;
  if (historyTokens > available) {
    for (let index = 0; index < view.length; index += 1) {
      ...
      const content = `[truncated ${original.length} chars]`;
      view[index] = { ...message, content };
      prunedChars += Math.max(0, original.length - content.length);
      historyTokens = estimateMessages(view, charsPerToken);
    }
  }
  return { messages: view, result: {
...
export function applyContextBudget(
  ...
  return contextBudgetView(messages, systemPrompt, tools, contextWindow, budget, charsPerToken).result;
}
