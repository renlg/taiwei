...
test('CommandSecurity 按用户隔离会话记住;guest 的永久记住降级为会话,不写全局白名单', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'taiwei-commsec-'));
  const oldHome = process.env.TAIWEI_HOME;
  process.env.TAIWEI_HOME = directory;
  try {
    const sec = new CommandSecurity();
    const security = { enabled: true, patterns: [] as string[], timeoutSeconds: 60, remember: 'off' as const, approvedPatterns: [] as string[] };
    const danger = 'rm -rf ./build';
    assert.ok(detectDanger(danger), '前置:命令必须命中危险正则');
    // 无 handler → 拒绝
    assert.equal(await sec.authorize('alice', danger, directory, security), false);
    // 用户 alice 允许并「本次会话记住」→ 再次放行
    assert.equal(await sec.authorize('alice', danger, directory, security, () => Promise.resolve({ approve: true, remember: 'session' as const })), true);
    assert.equal(await sec.authorize('alice', danger, directory, security), true, 'alice 本次会话内不再问');
    // 用户 bob 没有 alice 的会话记住 → 仍需确认(无 handler 拒绝)
    assert.equal(await sec.authorize('bob', danger, directory, security), false, 'bob 不受 alice 的会话记住影响');
    // guest(canWritePermanent=false)选「永久」→ 不写全局 approvedPatterns,只记会话
    const guestSec = new CommandSecurity();
    const guestApproved: string[] = [];
    const guestCfg = { enabled: true, patterns: [] as string[], timeoutSeconds: 60, remember: 'permanent' as const, approvedPatterns: guestApproved };
    const res = await guestSec.authorize('guest-user', danger, directory, guestCfg, () => Promise.resolve({ approve: true, remember: 'permanent' as const }), undefined, false);
    assert.equal(res, true, 'guest 允许成功');
    assert.equal(guestApproved.length, 0, 'guest 的「永久」不得写全局白名单');
    // 但 guest 本次会话记住生效
    assert.equal(await guestSec.authorize('guest-user', danger, directory, guestCfg), true, 'guest 会话内不再问');
    // 另一个 guest 用户不受影响
    assert.equal(await guestSec.authorize('guest-other', danger, directory, guestCfg), false, '其他 guest 仍需要确认');
    // admin(canWritePermanent=true)选「永久」→ 写全局白名单(磁盘 config)
    const adminCfg = { enabled: true, patterns: [] as string[], timeoutSeconds: 60, remember: 'permanent' as const, approvedPatterns: [] as string[] };
    const adminSec = new CommandSecurity();
    const adminRes = await adminSec.authorize('admin', danger, directory, adminCfg, () => Promise.resolve({ approve: true, remember: 'permanent' as const }), undefined, true);
    assert.equal(adminRes, true);
    const saved = await loadConfig();
    const savedPattern = detectDanger(danger)?.pattern;
    assert.ok(savedPattern && saved.security.approvedPatterns.includes(savedPattern), 'admin 的「永久」把命中正则写入全局白名单(磁盘)');
  } finally {
    if (oldHome === undefined) delete process.env.TAIWEI_HOME; else process.env.TAIWEI_HOME = oldHome;
    await rm(directory, { recursive: true, force: true });
  }
...
test('guest history tools only return sessions owned by the caller identity', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'taiwei-history-scope-test-'));
  const oldHome = process.env.TAIWEI_HOME;
  process.env.TAIWEI_HOME = directory;
  try {
    await upsertSession({ id: 'admin-session', title: 'Admin', source: 'gateway', ownerIdentity: 'admin' });
    await appendMessage({ sessionId: 'admin-session', role: 'user', content: 'classified admin content' });
    await upsertSession({ id: 'alice-session', title: 'Alice', source: 'delegate', ownerIdentity: 'alice' });
    await appendMessage({ sessionId: 'alice-session', role: 'user', content: 'classified alice content' });
    await upsertSession({ id: 'bob-session', title: 'Bob', source: 'delegate', ownerIdentity: 'bob' });
    await appendMessage({ sessionId: 'bob-session', role: 'user', content: 'classified bob content' });

    const registry = new ToolRegistry();
    for (const tool of createHistoryTools()) registry.register(tool);
    const context = { cwd: directory, workspaceRoot: directory, role: 'guest' as const, identity: 'alice', sessionId: 'alice-current' };
    const listed = JSON.parse(await registry.dispatch('session_list', {}, context)) as Array<{ sessionId: string }>;
    assert.deepEqual(listed.map((session) => session.sessionId), ['alice-session']);
    const searched = JSON.parse(await registry.dispatch('session_search', { query: 'classified' }, context)) as Array<{ sessionId: string }>;
    assert.deepEqual(searched.map((result) => result.sessionId), ['alice-session']);
    assert.deepEqual(JSON.parse(await registry.dispatch('session_get', { sessionId: 'admin-session' }, context)), { error: 'History session not found: admin-session' });
    assert.deepEqual(JSON.parse(await registry.dispatch('session_get', { sessionId: 'bob-session' }, context)), { error: 'History session not found: bob-session' });
    const own = JSON.parse(await registry.dispatch('session_get', { sessionId: 'alice-session' }, context)) as { messages: Array<{ content: string }> };
    assert.equal(own.messages[0]?.content, 'classified alice content');

    assert.deepEqual((await listSessions()).map((session) => session.sessionId).sort(), ['admin-session', 'alice-session', 'bob-session']);
  } finally {
    await closeHistoryDatabases();
    if (oldHome === undefined) delete process.env.TAIWEI_HOME; else process.env.TAIWEI_HOME = oldHome;
    await rm(directory, { recursive: true, force: true });
